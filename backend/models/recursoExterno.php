...
enum RecursoexternoError : string
{
...
class Recursoexterno extends BaseModel
{
    ...
    */
    const SQL_DEFAULT = "01999b0e-df5c-77d2-ad7d-7ee43f3a8776";
	    ...
	    if(!$stmt->execute())
	    {
	        ...
	        if($stmt->errno == 1062)
	        {
	            ...
	            if (preg_match("/Duplicate entry '(.+)' for key '(.+)'/", $msg, $matches))
	            {
					...
					{
					    $stmt->close();
					    return RecursoexternoError::DUPLICATE_ID_RECURSOEX;
					}
	                // Alternativa en caso de que no haya un código de error para la entrada duplicada específica.
	                $stmt->close();
	                return RecursoexternoError::UNKNOWN_DUPLICATE;
	            }
	        }
	...
	*/
	public static function getById(mysqli $con, int $idRecursoEx) : Recursoexterno|RecursoexternoError|ErrorDB
	{
	    ...
	    if($result->num_rows === 0)
	    {
	        $result->free();
	        $stmt->close();
	        return RecursoexternoError::NOT_FOUND;
	    }
