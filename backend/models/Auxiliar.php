...
enum AuxiliarErrorType : string
{
    case NOT_FOUND = "AUXILIAR_NOT_FOUND";
    case UNKNOWN_DUPLICATE = "AUXILIAR_UNKNOWN_DUPLICATE";
...
class AuxiliarError extends ErrorBase
{
    ...
    public static function unknownDuplicate(array $pair) : AuxiliarError
    {
        return new self(AuxiliarErrorType::UNKNOWN_DUPLICATE, $pair);
    }


    public static function duplicateIdAuxiliar(mixed $pair) : AuxiliarError
    {
        return new self(AuxiliarErrorType::DUPLICATE_ID_AUXILIAR, $pair);
    }
...
*/
class Auxiliar extends BaseModel
{
    /**
    ...
    */
    const SQL_DEFAULT = "0199b279-8ed8-72eb-8540-e90cf8dffc89";
	...
	*/
	protected function __construct(mysqli $con, int $idFuncionario, int $idAuxiliar)
	{
	    $this->con = $con;
	...
	*/
	public static function create(mysqli $con, int $idFuncionario, string|int $idAuxiliar = self::SQL_DEFAULT) : Auxiliar|AuxiliarError|ErrorDB
	{
		...
		// Añade 'Id_auxiliar' a la consulta de inserción SQL, solo si el parámetro
		// no indica que se debe utilizar el valor predeterminado de la base de datos (self::SQL_DEFAULT).
		if ($idAuxiliar !== self::SQL_DEFAULT)
		{
		    $columns[] = "Id_auxiliar";
		    $placeholders[] = "?";
		    $values[] = $idAuxiliar;
	    ...
	    if (empty($columns)) {
	        return ErrorDB::noValues('Auxiliar');
	    }
	
	    // Preparacion
	    $sql = "INSERT INTO Auxiliar (" . implode(", ", $columns) . ") VALUES (" . implode(", ", $placeholders) . ")";
	    ...
	    if(!$stmt->execute())
	    {
	        ...
	        if($stmt->errno == 1062)
	        {
	            ...
	            if (preg_match("/Duplicate entry '(.+)' for key '(.+)'/", $msg, $matches))
	            {
					...
					if($duplicateKey == 'Id_auxiliar')
					{
					    $stmt->close();
					    return AuxiliarError::duplicateIdAuxiliar(["column" => $duplicateKey, "value" => $duplicateValue]);
					}
	                // Alternativa en caso de que no haya un código de error para la entrada duplicada específica.
	                $stmt->close();
	                return AuxiliarError::unknownDuplicate(["column" => $duplicateKey, "value" => $duplicateValue]);
	            }
	        }
	    ...
	    // Auxiliar se ha creado exitosamente.
	    $stmt->close();
	...
	#region GET
	/**
	* Obtiene un Auxiliar de la base de datos identificando Auxiliar por su clave primaria única. 
	* Take into account that any attribute of type BLOB will store the base64 encoding of the blob.
	*/
	public static function getById(mysqli $con, int $idAuxiliar) : Auxiliar|AuxiliarError|ErrorDB
	{
	    // Preparar
	    $sql = "SELECT * FROM Auxiliar WHERE Id_auxiliar = ?";
	    $stmt = $con->prepare($sql);
	    if(!$stmt) return ErrorDB::prepare($sql);
	
	    // Vincular
	    $stmt->bind_param("i", $idAuxiliar);
	...
	/**
	* Obtiene el valor Id_funcionario de Auxiliar de la base de datos identificando Auxiliar por su clave primaria única. 
	*/
	public static function getIdFuncionarioById(mysqli $con, int $idAuxiliar) : int|AuxiliarError|ErrorDB
	{
	    // Preparar
	    $sql = "SELECT Id_funcionario FROM Auxiliar WHERE Id_auxiliar = ?";
	    $stmt = $con->prepare($sql);
	    if(!$stmt) return ErrorDB::prepare($sql);
	...
	/**
	* Establece el valor Id_funcionario de Auxiliar a partir de la base de datos que identifica Auxiliar por su clave principal. 
	*/
	public static function setIdFuncionarioById(mysqli $con, int $idAuxiliar, int $newIdFuncionario) : true|AuxiliarError|ErrorDB
	{
	    // Preparacion
	    $sql = "UPDATE Auxiliar SET Id_funcionario = ? WHERE Id_auxiliar = ?";
	    $stmt = $con->prepare($sql);
	    if(!$stmt) return ErrorDB::prepare($sql);
	
	    // Vinculacion
	    $null = null;
	    $stmt->bind_param("ii", $newIdFuncionario, $idAuxiliar);
	...
	/**
	* Borra un Auxiliar de la base de datos identificando Auxiliar por su clave primaria única.
	*/
	public static function deleteById(mysqli $con, int $idAuxiliar) : true|AuxiliarError|ErrorDB
	{
	    // Preparar
	    $sql = "DELETE FROM Auxiliar WHERE Id_auxiliar = ?";
	    $stmt = $con->prepare($sql);
	    if(!$stmt) return ErrorDB::prepare($sql);
