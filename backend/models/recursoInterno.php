...
enum RecursointernoError : string
{
...
class Recursointerno extends BaseModel
{
    ...
    */
    const SQL_DEFAULT = "01999b0e-df53-7941-b310-227c52307d90";
	    ...
	    if(!$stmt->execute())
	    {
	        ...
	        if($stmt->errno == 1062)
	        {
	            ...
	            if (preg_match("/Duplicate entry '(.+)' for key '(.+)'/", $msg, $matches))
	            {
					...
					{
					    $stmt->close();
					    return RecursointernoError::DUPLICATE_ID_RECURSOIN;
					}
	                // Alternativa en caso de que no haya un código de error para la entrada duplicada específica.
	                $stmt->close();
	                return RecursointernoError::UNKNOWN_DUPLICATE;
	            }
	        }
	...
	*/
	public static function getById(mysqli $con, int $idRecursoIn) : Recursointerno|RecursointernoError|ErrorDB
	{
	    ...
	    if($result->num_rows === 0)
	    {
	        $result->free();
	        $stmt->close();
	        return RecursointernoError::NOT_FOUND;
	    }
