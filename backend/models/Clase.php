...
abstract class ClaseError extends ErrorBase
{
    ...
    public static function unknownDuplicate(array $pair) : ClaseError
    {
        return new self(ClaseErrorType::UNKNOWN_DUPLICATE, $pair);
    }


    public static function duplicateIdClase(mixed $pair) : ClaseError
    {
        return new self(ClaseErrorType::DUPLICATE_ID_CLASE, $pair);
    }
...
class Clase extends BaseModel
{
	    ...
	    if(!$stmt->execute())
	    {
	        ...
	        if($stmt->errno == 1062)
	        {
	            ...
	            if (preg_match("/Duplicate entry '(.+)' for key '(.+)'/", $msg, $matches))
	            {
					...
					{
					    $stmt->close();
					    return ClaseError::duplicateIdClase(["column" => $duplicateKey, "value" => $duplicateValue]);
					}
	                // Alternativa en caso de que no haya un código de error para la entrada duplicada específica.
	                $stmt->close();
	                return ClaseError::unknownDuplicate(["column" => $duplicateKey, "value" => $duplicateValue]);
	            }
	        }
	...
	*/
	...
	/**
	* Borra un Clase de la base de datos identificando Clase por su clave primaria única.
	*/
	public static function deleteById(mysqli $con, int $idClase) : true|ClaseError|ErrorDB
	{
	    // Preparar
