...
enum AdscriptaErrorType : string
{
...
class AdscriptaError extends ErrorBase
{
    ...
    public static function unknownDuplicate(array $pair) : AdscriptaError
    {
        return new self(AdscriptaErrorType::UNKNOWN_DUPLICATE, $pair);
    }


    public static function duplicateIdAdscripta(mixed $pair) : AdscriptaError
    {
        return new self(AdscriptaErrorType::DUPLICATE_ID_ADSCRIPTA, $pair);
    }
...
class Adscripta extends BaseModel
{
    ...
    */
    const SQL_DEFAULT = "0199bae7-2d1d-720f-bfe1-4f98f9064fa8";
	...
	*/
	public static function create(mysqli $con, int $idFuncionario, string|int $idAdscripta = self::SQL_DEFAULT) : Adscripta|AdscriptaError|ErrorDB
	{
	    ...
	    if (empty($columns)) {
	        return ErrorDB::noValues('Adscripta');
	    }
	    ...
	    if(!$stmt->execute())
	    {
	        ...
	        if($stmt->errno == 1062)
	        {
	            ...
	            if (preg_match("/Duplicate entry '(.+)' for key '(.+)'/", $msg, $matches))
	            {
					...
					{
					    $stmt->close();
					    return AdscriptaError::duplicateIdAdscripta(["column" => $duplicateKey, "value" => $duplicateValue]);
					}
	                // Alternativa en caso de que no haya un código de error para la entrada duplicada específica.
	                $stmt->close();
	                return AdscriptaError::unknownDuplicate(["column" => $duplicateKey, "value" => $duplicateValue]);
	            }
	        }
	...
	*/
	public static function getById(mysqli $con, int $idAdscripta) : Adscripta|AdscriptaError|ErrorDB
	{
	    ...
	    if($result->num_rows === 0)
	    {
	        $result->free();
	        $stmt->close();
	        return AdscriptaError::notFound();
	    }
	...
	*/
	...
	/**
	* Borra un Adscripta de la base de datos identificando Adscripta por su clave primaria única.
	*/
	public static function deleteById(mysqli $con, int $idAdscripta) : true|AdscriptaError|ErrorDB
	{
	    // Preparar
	    $sql = "DELETE FROM Adscripta WHERE Id_adscripta = ?";
	    $stmt = $con->prepare($sql);
	    if(!$stmt) return ErrorDB::prepare($sql);
