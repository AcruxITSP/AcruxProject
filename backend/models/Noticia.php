...
class NoticiaError extends ErrorBase
{
    ...
    public static function unknownDuplicate(array $pair) : NoticiaError
    {
        return new self(NoticiaErrorType::UNKNOWN_DUPLICATE, $pair);
    }


    public static function duplicateIdNoticia(mixed $pair) : NoticiaError
    {
        return new self(NoticiaErrorType::DUPLICATE_ID_NOTICIA, $pair);
    }
...
class Noticia extends BaseModel
{
    ...
    */
    const SQL_DEFAULT = "0199b279-8e97-7cff-a9bc-61aafc6ca141";
	...
	*/
	public static function create(mysqli $con, string $fechaHora, string $contenido, ?int $idAdscripta, string|int $idNoticia = self::SQL_DEFAULT) : Noticia|NoticiaError|ErrorDB
	{
	    ...
	    if (empty($columns)) {
	        return ErrorDB::noValues('Noticia');
	    }
	    ...
	    if(!$stmt->execute())
	    {
	        ...
	        if($stmt->errno == 1062)
	        {
	            ...
	            if (preg_match("/Duplicate entry '(.+)' for key '(.+)'/", $msg, $matches))
	            {
					...
					{
					    $stmt->close();
					    return NoticiaError::duplicateIdNoticia(["column" => $duplicateKey, "value" => $duplicateValue]);
					}
	                // Alternativa en caso de que no haya un código de error para la entrada duplicada específica.
	                $stmt->close();
	                return NoticiaError::unknownDuplicate(["column" => $duplicateKey, "value" => $duplicateValue]);
	            }
	        }
	...
	*/
	...
	/**
	* Borra un Noticia de la base de datos identificando Noticia por su clave primaria única.
	*/
	public static function deleteById(mysqli $con, int $idNoticia) : true|NoticiaError|ErrorDB
	{
	    // Preparar
