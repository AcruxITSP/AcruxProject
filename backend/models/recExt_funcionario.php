...
enum RecextFuncionarioError : string
{
...
class RecextFuncionario extends BaseModel
{
    ...
    */
    const SQL_DEFAULT = "01999b0e-df61-758e-8b9d-2bfba82322a2";
	    ...
	    if(!$stmt->execute())
	    {
	        ...
	        if($stmt->errno == 1062)
	        {
	            ...
	            if (preg_match("/Duplicate entry '(.+)' for key '(.+)'/", $msg, $matches))
	            {
					...
					{
					    $stmt->close();
					    return RecextFuncionarioError::DUPLICATE_ID_REGISTRO;
					}
	                // Alternativa en caso de que no haya un código de error para la entrada duplicada específica.
	                $stmt->close();
	                return RecextFuncionarioError::UNKNOWN_DUPLICATE;
	            }
	        }
	...
	*/
	public static function getById(mysqli $con, int $idRegistro) : RecextFuncionario|RecextFuncionarioError|ErrorDB
	{
	    ...
	    if($result->num_rows === 0)
	    {
	        $result->free();
	        $stmt->close();
	        return RecextFuncionarioError::NOT_FOUND;
	    }
