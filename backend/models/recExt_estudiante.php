...
enum RecextEstudianteError : string
{
...
class RecextEstudiante extends BaseModel
{
    ...
    */
    const SQL_DEFAULT = "01999b0e-df5f-7ebb-871c-8edd45dd866d";
	    ...
	    if(!$stmt->execute())
	    {
	        ...
	        if($stmt->errno == 1062)
	        {
	            ...
	            if (preg_match("/Duplicate entry '(.+)' for key '(.+)'/", $msg, $matches))
	            {
					...
					{
					    $stmt->close();
					    return RecextEstudianteError::DUPLICATE_ID_REGISTRO;
					}
	                // Alternativa en caso de que no haya un código de error para la entrada duplicada específica.
	                $stmt->close();
	                return RecextEstudianteError::UNKNOWN_DUPLICATE;
	            }
	        }
	...
	*/
	public static function getById(mysqli $con, int $idRegistro) : RecextEstudiante|RecextEstudianteError|ErrorDB
	{
	    ...
	    if($result->num_rows === 0)
	    {
	        $result->free();
	        $stmt->close();
	        return RecextEstudianteError::NOT_FOUND;
	    }
